...
def savenota():
    ...
    pathx = os.path.join(app.root_path, 'static', 'notas', pathfinal,'notas.txt')
    print(pathx)
    with open(pathx, 'w',encoding='utf-8-sig') as f:
       f.write(datos)
...
def damediagramas():
    ...
    for cadafile in onlyfiles:
        ...
        losfiles += '<tr><td style="margin-right:20px;border: 1px solid black;" >' + cadafile + '</td><td style="margin-right:20px;border: 1px solid black;"  > <a  target="_blank" href="/static/diagramas/index.html?diagrama=' + cadafile + '" >Editar Diagrama</a> </td><td  style="margin-right:20px;border: 1px solid black;"   > <a href=''javascript:deldiagram("deletediagram?filename=' + cadafile + '"'') >Eliminar Digrama</a> </td></tr>'
...
def damefiles():
    ...
    pathx = os.path.join(app.root_path, 'static', 'notas', tabla, idobjeto,'notas.txt')
    if not os.path.isfile(pathx):
        f = open(  pathx  , "w",encoding='utf8')
        f.write('')
        f.close()
    ...
    return losfiles + '</table><br><h5>Notas</h5> <iframe id="lasnotas" style="width: 95%;height: 400px;"  src="/static/editor/index.html?ruta=' + tabla + '*' + idobjeto + '"/>'
